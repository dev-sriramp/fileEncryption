      ...
      try {
        ...
        if (file.size < 1024 * 1024 * 2 * 1.8 && password.value !== "") {
          try {
            let reader = new FileReader();
            reader.onload = function(e) {
              let decrypted = CryptoJS.AES.decrypt(e.target.result, password.value).toString(CryptoJS.enc.Latin1);
  ...
  return (<div>
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <div id="loginform">
          <button type="button" className="btn btn-success Size" onClick={(e) => {
              setType("Encrypt");
              setHrefOfFile(null);
              setDownload(null)
            }}>Encrypt</button>
          <button type="button" className="btn btn-danger float-end Size" onClick={(e) => {
              setType("Decrypt");
              setHrefOfFile(null);
              setDownload(null)
            }}>Decrypt</button>
      ...
      <center>
        {
          high && <h3 className="text-white">
              Download Our Desktop Application to process
              <a href={linkToLargeFile} target="_blank" rel="noreferrer">Large Files</a>
            </h3>
        }</center>
    </div>
